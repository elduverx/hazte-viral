...
### Opción 3: Build Settings para Archive/App Store (obligatorio para revisión)

Esta es la configuración que usa el binario que subes a App Store Connect.

1. Abre el proyecto en Xcode y selecciona el target **Hazte Viral**
2. Ve a **Build Settings**
3. Busca `ANTHROPIC_API_KEY`
4. Define un valor real para **Release** (y recomendado también en Debug)
5. Verifica que `Info.plist` tenga:
   ```xml
   <key>AnthropicAPIKey</key>
   <string>$(ANTHROPIC_API_KEY)</string>
   ```
6. Antes de archivar, valida con:
   ```bash
   ./scripts/verify_release_api_key.sh
   ```
...
- Para TestFlight/App Store, usa también la **Opción 3** (Build Settings de Release)
